...
//xss过滤（修复get_magic_quotes_gpc()错误）
function stripStr($str){
    // 移除PHP7.4+已删除的get_magic_quotes_gpc()判断
    // 直接返回过滤后的值
...
function isBase64Formatted($str)
{
	if(preg_match('/^[A-Za-z0-9+\/=]+$/', $str)) {
		...
		if ($str == base64_encode($decoded)) {
			if(preg_match('/^[A-Za-z0-9\x00-\x80~!@#$%&_+-=:";\'<>,\/"\[\]\\\^\.\|\?\*\+\(\)\{\}\s]+$/', $decoded)) {
				return true;
			}
		}
	}
...
//基于Discuz X3.1 function_misc.php的IP解析
